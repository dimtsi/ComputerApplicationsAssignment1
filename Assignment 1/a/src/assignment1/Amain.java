...
public class Amain {
	...
	public static void main(String[] args){
		...
		//BaseVoltage
		NodeList BVlList = file.doc.getElementsByTagName("cim:BaseVoltage"); //Creates nodelist of Base voltages
		ArrayList<BaseVoltage> BV = new ArrayList<BaseVoltage>(); // Create arraylist of parsed based voltages
		
		for (int i = 0; i < BVlList.getLength(); i++) { //fills arraylist(goto function extractBV, class BaseVoltage)
	...
	//////////////////////////////////////////////////////////////////
		...
		Transl = CreateTranslation(Term , BBs , Breakers , ACLS , PTrsf); //Arraylist of components used in Ybus and the Terminal IDs they are connected to
		...
		int term1 , term , term2 ,term3 , term4 , term5 , term6 , ConnNode , ConnNode1 ;
		int isocount ;
		boolean isolation;
		isocount = 0;
		Complex Y, Ysh;
		...
		for (int i = 0; i < BBs.size(); i++){
			...
			for (int j = 0 ; j < ConNodeTerms.get(ConnNode).GetList().size() ; j++){ //terminals connected to CN
				...
				case "Breaker":
					...
					if (check.get(term2).GetCheck()) {  //have I been here before only through breakers?
						samebus.add(new Integer[]{i , check.get(term2).GetBusID()}); }
					Check e = new Check(true , i); //say I came through this terminal starting from this bus
					check.set(term2 , e);  
					...
					for (int l = 0; l < ConNodeTerms.get(ConnNode1).GetList().size() ; l++){//terminals connected to CN
						...
						if (term5 == term2){ continue; } //skip the terminal that you came from
						if (!check(term5 , Transl)){ continue; } // check if element of Ybus 
						if (Transl.get(term5)[1].equals("PowerTransformer")){ //chech for PT after breaker
							Check f = new Check(true , i);
							check.set(term5 , f);
							Y = PTrsf.get(Integer.parseInt(Transl.get(term5)[2])).GetY(); //get Y of transformer
							Ysh = PTrsf.get(Integer.parseInt(Transl.get(term5)[2])).Getysh(); //get Yshunt of transformer
							term6 = PTrsf.get(Integer.parseInt(Transl.get(term5)[2])).GetTerm1(); //get Transformer Terminal
							if (term5 == term6){term6 = PTrsf.get(Integer.parseInt(Transl.get(term5)[2])).GetTerm2(); }	//Check if I have been here before
							if (check.get(term6).GetCheck()) { //add in Ybus the route from Bus i to the bus that we reached
								Ybus x3 = new Ybus(Y , Ysh , i , check.get(term6).GetBusID());
								Yb.add(x3); //Bus depicting the routes in Y-bus as well as the series and shunt admittances of the routes
								isolation = false; //if a rout exists there is no isolation
								continue;
							}
			...
			if (isolation) { //check if a bus is isolated
		...
		System.out.println("Total buses isolated: "+isocount+"\n");
...
//Create the Ybus
		...
		ArrayList<Help> help = new ArrayList<Help>(); // has the BusID of ,breaker only, connected
													  // buses and the buses IDs (bus ID) connected to it
		...
		for (int i = 0 ; i < Yb.size() ; i++){ // modify Ybus so that it only contains real buses IDs, merge breaker-only connected buses
		...
		startList = new ArrayList<Integer>(); //Bus IDs in Ybus and their IDs
		flag = false;
		for (int i = 0 ; i < Yb.size() ; i++){
			...
			for(int j = i+1 ; j < Yb.size(); j++){ //check for double connections
				if (((Yb.get(i).GetBusID1() == Yb.get(j).GetBusID1()) && (Yb.get(i).GetBusID2() == Yb.get(j).GetBusID2())) || ((Yb.get(i).GetBusID2() == Yb.get(j).GetBusID1()) && (Yb.get(i).GetBusID1() == Yb.get(j).GetBusID2())) ){
					...
					Yb.set(i, x); //merge double connections
					Yb.remove(j);
				}
			}
		...
		QuickSort sorter = new QuickSort(); //Sort IDs   ***************** FIRST PARSED FIRST SERVED :) :)*******************
		sorter.sort(startList);

		
		ArrayList<Integer[]>newstartlist = new ArrayList<Integer[]>();
		for (int i = 0; i < startList.size(); i++){
			newstartlist.add(new Integer[]{i , startList.get(i) });
		}
		//iniatilize Yfinal
		Complex[][] Yf = new Complex[size][size];
		for (int i = 0 ; i < size ; i++){
			for (int j = 0; j < size ; j++){
				Yf[i][j] = new Complex(0,0);
			}
		}
		//fill the Yfinal
		...
		for (int i = 0 ; i <size ; i++){
			...
			for (int j = 0 ; j < Yb.size() ; j++){
				if (newstartlist.get(i)[1] == Yb.get(j).GetBusID1()){ 
					Yf[i][i] = Yf[i][i].plus(Yb.get(j).GetY().plus(Yb.get(j).Gety())); //fill diagonal elements
					temporary = Yb.get(j).GetBusID2();
					Yf[i][temporary] = Yb.get(j).GetY().multi(-1);// fill non diagonal elements
					Yf[temporary][i] = Yb.get(j).GetY().multi(-1);// fill non diagonal elements
				}
				if (newstartlist.get(i)[1] == Yb.get(j).GetBusID2()){ Yf[i][i] = Yf[i][i].plus(Yb.get(j).GetY().plus(Yb.get(j).Gety()));} //fill diagonal elements
					
			}
		}
					

//print the Ybus (Yfinal)
		System.out.println("There are:"+size+" buses in the Ybus\n");
		System.out.println("Ybus:\n");
		
		for (int i = 0 ; i <size ; i++){ 
			System.out.print("Line "+ (i+1) +"\n\n");
			for (int j = 0 ; j< size ; j++){
				System.out.println(Yf[i][j].StringRep());
			}
			System.out.println("");
		}
	...
	public static void TableExample(Complex YFINAL[][]){ //Creates Y-Matrix in GUI
		String [][] Yprinted = new String [YFINAL.length][YFINAL.length];
        ...
        panel.setBorder (BorderFactory.createTitledBorder (BorderFactory.createEtchedBorder (),
                                                            "Y-MATRIX (All Elements in pu)",
                                                            TitledBorder.CENTER,
                                                            TitledBorder.TOP));

	    for (int i = 0 ; i <YFINAL.length ; i++){ 
	    	for (int j = 0 ; j< YFINAL.length ; j++){
	    		Yprinted [i][j] = YFINAL[i][j].StringRep();	    			
	    	}
	    }	
	    
	    String data[][]= Yprinted;  
	    String columns[]= new String[YFINAL.length]; 
	    for (int i = 0 ; i <YFINAL.length ; i++){
	...
	public static BaseVoltage extractBV (Node node){
		...
		String rdfID = element.getAttribute("rdf:ID"); // get The rdfID input
		if (rdfID == null){
			System.out.println("Error. Input other EQ file"); // if there isn't in the file stop the programm
			System.exit(0);
		}
	...
	public static Substation extractSub (Node node){
		...
		if (rdfID == null){
			System.out.println("Error. Input other EQ file");
			System.exit(0);
		}
	...
	public static SynchronousMachine extractSM (Node nodeEQ , Node nodeSSH){
		...
		Element element1 = (Element) nodeEQ; //Nodelist of SMs from EQ File
		Element element2 = (Element) nodeSSH; //Nodelist of SMs from SSH File
	
		String EqC , RC , RMGU , name , S , P , Q;  //Attributes that we want to save
		...
		try{
			...
			for (int i = 0; i < childList.getLength(); i++) { //Find childlists that we should enter in order to obtain attributes
				Node childNode = childList.item(i);
				if ("cim:Equipment.EquipmentContainer".equals(childNode.getNodeName())) {
		...
		SynchronousMachine x = new SynchronousMachine (rdfID , name , EqC , RC , RMGU , S , P , Q);  //Add object to the arraylist
		return x;
	}
	...
	public static void CreateDatabase (ArrayList<BaseVoltage> bv , ArrayList<Substation> sub , ArrayList<VoltageLevel> vl ,
		...
		String PASS = JOptionPane.showInputDialog("Enter your SQL server password"); // insert the password to SQL server
